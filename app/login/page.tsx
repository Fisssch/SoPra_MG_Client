...
const Login: React.FC = () => {
	...
	const handleLogin = async (values: UserDTO) => {
		...
		try {
			...
			if (error?.status === 404) {
				message.warning('No user found with given username.');
	...
	return (
		<div
			...
			<div className='login-container'>
				...
				<Form form={form} name='login' size='large' variant='outlined' onFinish={handleLogin} layout='vertical'>
					<Form.Item name='username' label={<span className='text-black'>Username</span>} rules={[{ required: true, message: 'Please input your username!' }]}>
						<Input placeholder='Enter username' />
					</Form.Item>
					<Form.Item name='password' label={<span className='text-black'>Password</span>} rules={[{ required: true, message: 'Please input your password!' }]}>
						<Input.Password

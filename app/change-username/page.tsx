...
const EditUsername: React.FC = () => {
	...
	const handleUpdate = async () => {
		...
		try {
			await apiService.put<void>(`/users/${userId}/username`, { username: newUsername }, { Authorization: `Bearer ${token}` });

			message.success('Username updated successfully!');
			router.push(`/mainpage`);
		} catch (error: any) {
			if (error?.status === 403) {
				message.error('You can only change your own username.');
	...
	return (
		<div
			...
			<div className='login-container' style={{ maxWidth: '325px', boxShadow: '0 8px 24px rgba(0, 0, 0, 1)' }}>
				<h2 className='text-3xl font-semibold text-center mb-4 text-black'>Change Username</h2>

				<Form name='edit-username' size='large' onFinish={handleUpdate} layout='vertical'>
					<Form.Item
						...
						<Input placeholder='Enter new username' className='bg-white text-black' value={newUsername} onChange={e => setNewUsername(e.target.value)} />
					</Form.Item>

					<Form.Item>
						<div style={{ display: 'flex', flexDirection: 'column', gap: '10px', alignItems: 'center' }}>
							<Button type='primary' htmlType='submit' style={{ width: '70%' }} size='middle'>
								Save Username
							</Button>
							<Button onClick={() => router.back()} style={{ width: '70%' }} size='middle'>
								Cancel
							</Button>
						</div>
					</Form.Item>

	...
	const handleGuess = async (word: string) => {
		...
		try {
			...
			setGameData(prevGameData => {
				...
				const updatedBoard = prevGameData.board.map(card => (card.word === word ? { ...card, guessed: true } : card));
		...
		<div
				...
				{teamColor === gameData.teamTurn ? (
					!isSpymaster ? (
						currentHint ? (
							...
							<h1 className='text-4xl font-bold text-white mt-6'>Wartet auf den Hinweis von eurem Spymaster...</h1>
						)
					) : !hintSubmitted ? (
						// Spymaster sees the input fields to provide a hint
						<div className='flex flex-col items-center gap-2 mt-6'>
							<p className='text-4xl font-bold mb-4'>Du bist dran, gib ein Hinweis und eine Zahl an!</p>
							<div className='flex items-center gap-4 mt-2'>
								...
								/>
								<button onClick={sendHint} className='bg-green-600 px-6 py-3 rounded text-lg font-semibold text-white'>
									Hinweis absenden
								</button>
							</div>
						</div>
						...
						<p className='text-4xl font-bold text-white mt-6'>Hinweis abgegeben! Warte auf die Reaktion des anderen Teams...</p>
					)
				) : (
					// Opposing team sees "it's the other team's turn"
					<p className='text-4xl font-bold mt-6'>Das andere Team ist dran...</p>
				)}
			...
			<div className='flex justify-between items-center w-full px-12 max-w-7xl mx-auto mb-10'>
				...
				<div className='bg-blue-700 h-32 w-40 p-4 rounded-xl flex flex-col justify-center items-center shadow-md border-4 border-blue-400 ml-4!'>
					<span className='text-3xl font-bold'>{gameData.board.filter(card => card.color === 'BLUE' && !card.guessed).length}</span>
				...
				<div className='bg-red-700 h-32 w-40 p-4 rounded-xl flex flex-col justify-center items-center shadow-md border-4 border-red-400 absolute right-4 '>
					<span className='text-3xl font-bold mt-5'>{gameData.board.filter(card => card.color === 'RED' && !card.guessed).length}</span>
					...
					{gameData.board.map((card, index) => {
						...
						return (
							<div
								key={index}
								onClick={() => {
									if (!card.guessed && !isSpymaster && teamColor === gameData.teamTurn) {
										if (!currentHint) {
